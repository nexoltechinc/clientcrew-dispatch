...
class TechnicianProfileApiTests(unittest.TestCase):
    ...
    def test_email_change_request_creates_pending_record(self):
        tech = self._seed_technician(name="Taylor Brooks", email="jolianne@dispatchiq.test")
        tech_auth = self._technician_auth_header(email=tech.email)

        res = self.client.post(
            "/technicians/me/email-change-request",
            json={"requested_email": "jolianne.updated@dispatchiq.test"},
            headers=tech_auth,
        )
        self.assertEqual(res.status_code, 201, res.text)
        payload = res.json()
        self.assertEqual(payload["status"], "PENDING")
        self.assertEqual(payload["current_email"], "jolianne@dispatchiq.test")
        self.assertEqual(payload["requested_email"], "jolianne.updated@dispatchiq.test")
        ...
        self.assertEqual(me_payload["email"], "jolianne@dispatchiq.test")
        self.assertTrue(me_payload["has_pending_email_change_request"])

    def test_admin_approve_updates_email(self):
        tech = self._seed_technician(name="Jordan Lee", email="victor@dispatchiq.test")
        tech_auth = self._technician_auth_header(email=tech.email)

        request_res = self.client.post(
            "/technicians/me/email-change-request",
            json={"requested_email": "victor.new@dispatchiq.test"},
            headers=tech_auth,
        )
        ...
        me_res = self.client.get("/technicians/me", headers=tech_auth)
        self.assertEqual(me_res.status_code, 200, me_res.text)
        self.assertEqual(me_res.json()["email"], "victor.new@dispatchiq.test")

    def test_admin_reject_does_not_update_email(self):
        tech = self._seed_technician(name="Casey Patel", email="maxime@dispatchiq.test")
        tech_auth = self._technician_auth_header(email=tech.email)

        request_res = self.client.post(
            "/technicians/me/email-change-request",
            json={"requested_email": "maxime.new@dispatchiq.test"},
            headers=tech_auth,
        )
        self.assertEqual(request_res.status_code, 201, request_res.text)
        request_id = request_res.json()["id"]
        ...
        me_res = self.client.get("/technicians/me", headers=tech_auth)
        self.assertEqual(me_res.status_code, 200, me_res.text)
        self.assertEqual(me_res.json()["email"], "maxime@dispatchiq.test")

    def test_integration_availability_and_email_request_visible_to_admin(self):
        tech = self._seed_technician(name="Riley Carter", email="dany@dispatchiq.test")
        tech_auth = self._technician_auth_header(email=tech.email)
        ...
        request_res = self.client.post(
            "/technicians/me/email-change-request",
            json={"requested_email": "dany.new@dispatchiq.test"},
            headers=tech_auth,
        )
        self.assertEqual(request_res.status_code, 201, request_res.text)
        request_id = request_res.json()["id"]
        ...
        self.assertEqual(pending_row["pending_email_change_requested_email"], "dany.new@dispatchiq.test")
        ...
        self.assertEqual(approved_row["email"], "dany.new@dispatchiq.test")
